...
contract Bidding{
   ...
   // SET ON BID FUNCTION ONLY OWNER//   
    ...
    function bid(address user, uint256 tokenId, uint256 price)public {
        require(child.ownerOf(tokenId) != user, " owner should not be allowed to bid on his own nft");
        require(user == msg.sender , "you can't bid on another address");
        require(highestBid[tokenId]<price,"Increase your bid someone already bid higher than you!");
